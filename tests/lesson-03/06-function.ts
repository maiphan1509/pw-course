...
let getTopStudents = (
  ...
  let topStudents = students.filter((student) => student.score > threshold);
  return topStudents;
};
console.log(
  `Students with scores above ${threshold}:`,
  getTopStudents(students, threshold).map((student) => student.name),
);
...
console.log(
  `4. The total amount after calculating interest is: ${interestResult}`,
);
